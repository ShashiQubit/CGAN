...
Original file is located at
    https://colab.research.google.com/drive/1Xw2w6H7MmUyk3Dpw-AtWkPLJe6y2I7rh
...
plt.savefig("plots/raw_series_of_S&P500 (CGAN).svg", format="svg")
plt.close()
...
# Show the plot
plt.savefig("plots/S&P 500 Index Direct Returns_and_S&P 500 Index Log Returns (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Gaussian_PDF_and_log_returned_density (CGAN).svg", format="svg")
plt.close()
...
plt.ylabel(r'$\rho$')
plt.grid()
plt.savefig("plots/ACF S&P 500 Log-Returns (CGAN).svg", format="svg")
plt.close()
...
plt.grid()
plt.savefig("plots/ACF S&P 500 Absolute Log-Returns (CGAN).svg", format="svg")
plt.close()
...
plt.ylabel(r'$L(\tau)$')
plt.grid()
plt.savefig("plots/Leverage Effect S&P 500 (CGAN).svg", format="svg")
plt.close()
...
<img src="https://github.com/dkomni/financial-modeling-gans/blob/master/images/gan.png?raw=1" width="60%" align="center">
...
plt.grid(True)
plt.savefig("plots/Q-Q Plot - S&P 500 Log Returns (CGAN).svg", format="svg")
plt.close()
...
axes.legend()
plt.savefig("plots/Gaussian_PDF_and_Normalized_density (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Gaussian_PDF_and_Transformed_log_returns (CGAN).svg", format="svg")
plt.close()
...
plt.ylabel('Sample Quantiles')
plt.grid(True)
plt.savefig("plots/Q-Q Plot - Transformed S&P 500 Log Returns (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Reverse_Gaussian_PDF_and_Transformed_log_returns (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Reverse_Gaussian_PDF_and_Normalized_density (CGAN).svg", format="svg")
plt.close()
...
plt.ylabel('Sample Quantiles')
plt.grid(True)
plt.savefig("plots/Q-Q Plot - Reversed pre-processed S&P 500 Log Returns (CGAN).svg", format="svg")
plt.close()
...
class GAN(tf.keras.Model):
    def __init__(self, num_epochs, batch_size, latent_dim, window_size, d_steps=1):
        super(GAN, self).__init__()
        self.critic = self.define_critic_model(window_size)   # will modify final activation inside
    ...
    # copy your existing define_critic_model but change final Dense to sigmoid activation
    def define_critic_model(self, window_length):
        model = tf.keras.Sequential()
        model.add(tf.keras.layers.Conv1D(filters=64, kernel_size=10, strides=1,
                                         input_shape=(window_length, 1), padding='same'))
        ...
        model.add(tf.keras.layers.Dense(1, activation='sigmoid', dtype=tf.float64))
        return model

    # copy your existing generator definition (no change)
    ...
    def compile_GAN(self, d_optimizer, g_optimizer):
        super(GAN, self).compile()
        self.d_optimizer = d_optimizer
        self.g_optimizer = g_optimizer
        self.bce = tf.keras.losses.BinaryCrossentropy(from_logits=False)

    def train_gan(self, gan_data_tf, original_data, preprocessed_data):
        """
        gan_data_tf : tensor of windows (like your rolling_window output)
        original_data : original log-returns (for metrics)
        preprocessed_data : preprocessed transformed data used in denormalize steps
        """
        # create batched dataset
        num_elements = len(gan_data_tf)
        ds = tf.data.Dataset.from_tensor_slices(gan_data_tf)
        ds = ds.shuffle(buffer_size=max(1000, num_elements)).batch(self.batch_size, drop_remainder=True)

        for epoch in range(self.num_epochs):
            d_losses = []
            g_losses = []
            for real_batch in ds:
                # shape transform: (batch_size, window) -> (batch_size, window, 1)
                real = tf.reshape(real_batch, (self.batch_size, self.window_size, 1))
                # perform d_steps discriminator updates
                for _ in range(self.d_steps):
                    noise = tf.random.normal((self.batch_size, self.latent_dim))
                    with tf.GradientTape() as d_tape:
                        fake = self.generator(noise)
                        fake_expanded = tf.reshape(fake, (self.batch_size, self.window_size, 1))

                        d_real = self.critic(real)           # probabilities
                        d_fake = self.critic(fake_expanded)

                        d_loss_real = self.bce(tf.ones_like(d_real), d_real)
                        d_loss_fake = self.bce(tf.zeros_like(d_fake), d_fake)
                        d_loss = d_loss_real + d_loss_fake

                    d_grads = d_tape.gradient(d_loss, self.critic.trainable_variables)
                    self.d_optimizer.apply_gradients(zip(d_grads, self.critic.trainable_variables))
                    d_losses.append(d_loss.numpy())

                # generator update (one step per batch)
                noise = tf.random.normal((self.batch_size, self.latent_dim))
                with tf.GradientTape() as g_tape:
                    fake = self.generator(noise)
                    fake_expanded = tf.reshape(fake, (self.batch_size, self.window_size, 1))
                    d_fake_for_g = self.critic(fake_expanded)
                    # non-saturating generator loss
                    g_loss = self.bce(tf.ones_like(d_fake_for_g), d_fake_for_g)
            ...
            # epoch averages
            self.critic_loss_avg.append(np.mean(d_losses))
            self.generator_loss_avg.append(np.mean(g_losses))

            # ----- evaluation on full-length generated series (same as in your WGAN code) -----
            ...
            # reverse preprocessing (same steps as your file)
            ...
            # compute metrics using your existing stylized_facts function if you preserve it,
            # otherwise compute directly here (example below uses same signature as your old method)
            corr_rmse, volatility_rmse, lev_rmse, emd = self._stylized_facts_local(original_data, fake_original)
            ...
            if (epoch + 1) % 20 == 0:
              self.generator.save_weights(f"checkpoints/generator_epoch_CGAN_{epoch+1}.weights.h5")
              self.critic.save_weights(f"checkpoints/critic_epoch_CGAN_{epoch+1}.weights.h5")


            # minimal console output (epoch numbers)
            if epoch % 50 == 0:
                print(f"Epoch {epoch+1}/{self.num_epochs} D_loss={self.critic_loss_avg[-1]:.4f} G_loss={self.generator_loss_avg[-1]:.4f} EMD={emd:.6f}")

    # local version of stylized_facts adapted inline (keeps same outputs as original)
    def _stylized_facts_local(self, original_data, fake_original):
        import statsmodels.api as sm
        import tensorflow_probability as tfp

        # generated acf
        acf_values_gen = sm.tsa.acf(np.array(fake_original), nlags=28)[1:]
        acf_values_orig = sm.tsa.acf(np.array(original_data), nlags=28)[1:]

        acf_abs_gen = sm.tsa.acf(np.abs(np.array(fake_original)), nlags=28)[1:]
        acf_abs_orig = sm.tsa.acf(np.abs(np.array(original_data)), nlags=28)[1:]

        # leverage
        lev_gen = []
        lev_orig = []
        for lag in range(1,29):
            r_t_gen = fake_original[:-lag]
            sqlag_gen = np.square(np.abs(fake_original[lag:]))
            lev_gen.append(tfp.stats.correlation(r_t_gen, sqlag_gen, sample_axis=0, event_axis=None).numpy())

            r_t_orig = original_data[:-lag]
            sqlag_orig = np.square(np.abs(original_data[lag:]))
            lev_orig.append(tfp.stats.correlation(r_t_orig, sqlag_orig, sample_axis=0, event_axis=None).numpy())

        rmse_acf = np.sqrt(np.mean((acf_values_orig - acf_values_gen)**2))
        rmse_vol = np.sqrt(np.mean((acf_abs_orig - acf_abs_gen)**2))
        rmse_lev = np.sqrt(np.mean((np.array(lev_orig) - np.array(lev_gen))**2))

        # EMD (use histograms like original)
        bin_edges = np.linspace(-0.05, 0.05, num=50)
        empirical_real, _ = np.histogram(np.array(original_data), bins=bin_edges, density=True)
        empirical_real /= np.sum(empirical_real)
        empirical_fake, _ = np.histogram(np.array(fake_original), bins=bin_edges, density=True)
...
gan = GAN(EPOCHS, BATCH_SIZE, LATENT_DIM, WINDOW_SIZE, d_steps=n_critic)

c_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001)
g_optimizer = tf.keras.optimizers.Adam(learning_rate=0.0001)
gan.compile_GAN(c_optimizer, g_optimizer)
...
gan.train_gan(gan_data_tf, sp500_log_r_tf, transformed_sp500)
exec_time_train = time.time() - start_time_train
print(f'\nGAN training completed. Training time: --- {exec_time_train/3600:.02f} hours ---')

"""## Plot Training History"""

critic_loss = np.array(gan.critic_loss_avg)
...
plt.tight_layout()
plt.savefig("plots/training_history (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Original Log-Returns_and_Generated Log-Returns (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/Original vs Generated Density (CGAN).svg", format="svg")
plt.close()
...
plt.savefig("plots/ACF Log-Returns (Generated) (CGAN).svg", format="svg")
plt.close()
...
axes[1].grid()
plt.savefig("plots/ACF Absolute Log-Returns (Generated) (CGAN).svg", format="svg")
plt.close()
...
axes[1].grid()
plt.savefig("plots/Generated Leverage Effect (CGAN).svg", format="svg")
plt.close()
...
sp500_from_csv = pd.read_csv('sp500_data.csv')

# Display the first few rows
print("First 5 rows of the loaded CSV data:")
display(sp500_from_csv.head())

# Convert the 'Close' column to numeric, coercing errors
sp500_from_csv['Close'] = pd.to_numeric(sp500_from_csv['Close'], errors='coerce')


# Check the data types of the columns
print("\nData types of the columns:")
display(sp500_from_csv.info())

# Check for missing values after conversion
print("\nMissing values per column after converting 'Close' to numeric:")
display(sp500_from_csv.isnull().sum())

# Check the data type of the 'Close' column specifically, as it will be used for tensors
print("\nData type of the 'Close' column after conversion:")
print(sp500_from_csv['Close'].dtype)
